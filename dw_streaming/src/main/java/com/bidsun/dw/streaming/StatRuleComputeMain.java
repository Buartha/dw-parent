 ...
 */
public class StatRuleComputeMain implements StatRuleComputeConstants {
    private static Logger logger = LoggerFactory.getLogger(StatRuleComputeMain.class);
       ...
       kaiBiaoJiLuCountStream.print();
        ...
        SingleOutputStreamOperator<GCJSPingBiaoBaoGao> gcjsPingBiaoBaoGaoStream = gcjsPingBiaoBaoGaoTagStream.keyBy("tableName").flatMap(new RichFlatMapFunction<EbidsunEvent, GCJSPingBiaoBaoGao>() {
            ...
            public void flatMap(EbidsunEvent ebidsunEvent, Collector<GCJSPingBiaoBaoGao> out) throws Exception {
                String eventBody = ebidsunEvent.getEventBody();
                try {
                    GCJSPingBiaoBaoGao GCJSPingBiaoBaoGao = JSON.parseObject(eventBody, com.bidsun.dw.pojo.GCJSPingBiaoBaoGao.class);
   ...
   PingBiaoBaoGaoCountStream.print();
        ...
        SingleOutputStreamOperator<GCJSTouBiaoBaoZhengJinJiaoNa> gcjsTouBiaoBaoZhengJinJiaoNaStream = gcjsTouBiaoBaoZhengJinJiaoNaTagStream.keyBy("tableName").flatMap(new RichFlatMapFunction<EbidsunEvent, GCJSTouBiaoBaoZhengJinJiaoNa>() {
            ...
            public void flatMap(EbidsunEvent ebidsunEvent, Collector<GCJSTouBiaoBaoZhengJinJiaoNa> out) throws Exception {
                String eventBody = ebidsunEvent.getEventBody();
                try {
                    GCJSTouBiaoBaoZhengJinJiaoNa GCJSTouBiaoBaoZhengJinJiaoNa = JSON.parseObject(eventBody, com.bidsun.dw.pojo.GCJSTouBiaoBaoZhengJinJiaoNa.class);
        ...
        SingleOutputStreamOperator<GCJSZhaoBiaoGongGao> gcjsZhaoBiaoGongGaoStream = gcjsZhaoBiaoGongGaoTagStream.keyBy("tableName").flatMap(new RichFlatMapFunction<EbidsunEvent, GCJSZhaoBiaoGongGao>() {
            ...
            public void flatMap(EbidsunEvent ebidsunEvent, Collector<GCJSZhaoBiaoGongGao> out) throws Exception {
                String eventBody = ebidsunEvent.getEventBody();
                try {
                    GCJSZhaoBiaoGongGao GCJSZhaoBiaoGongGao = JSON.parseObject(eventBody, com.bidsun.dw.pojo.GCJSZhaoBiaoGongGao.class);
...
//上链数据
        ...
        //统计收藏量
        SingleOutputStreamOperator<com.bidsun.dw.pojo.subscribe> subscribeStream = subscribeTagStream.keyBy("tableName").flatMap(new RichFlatMapFunction<EbidsunEvent, com.bidsun.dw.pojo.subscribe>() {
            @Override
            public void flatMap(EbidsunEvent ebidsunEvent, Collector<com.bidsun.dw.pojo.subscribe> out) throws Exception {
                String eventBody = ebidsunEvent.getEventBody();
                com.bidsun.dw.pojo.subscribe subscribe = new subscribe();
                String pl = "";
                try {
                    ...
                    if (null != trade_center) {
                        ...
                        subscribe = JSON.parseObject(eventBody, com.bidsun.dw.pojo.subscribe.class);
        ...
        SingleOutputStreamOperator<com.bidsun.dw.pojo.subscribe> subscribeStream_2 = subscribeStream.flatMap(new FlatMapFunction<com.bidsun.dw.pojo.subscribe, com.bidsun.dw.pojo.subscribe>() {
            @Override
            public void flatMap(com.bidsun.dw.pojo.subscribe line, Collector<com.bidsun.dw.pojo.subscribe> out) throws Exception {
                //将一行字符串按空格切分成一个字符串数组
        ...
        DataStream<AdsGzhDataApplicationDaily> subscribeCountStream = subscribeStream_2.filter(x -> x.getPlatform_code() != null)
                .keyBy(new KeySelector<com.bidsun.dw.pojo.subscribe, String>() {
                    @Override
                    public String getKey(com.bidsun.dw.pojo.subscribe value) throws Exception {
                        return value.getPlatform_code();
                    }
                })
                .timeWindow(Time.days(1), Time.seconds(10))
